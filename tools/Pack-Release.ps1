...
$patchC = Join-Path $ClientRoot 'Data\zhCN\patch-zhCN-c.MPQ'
if (-not (Test-Path -LiteralPath $patchC)) { throw "missing patch-zhCN-c.MPQ" }
Copy-Item -LiteralPath $patchC -Destination (Join-Path $zhStage 'patch-zhCN-c.MPQ')
...
$dll = Join-Path $ClientRoot 'plugins\SoloCam.dll'
...
Copy-Item (Join-Path $zhStage 'patch-zhCN-c.MPQ') (Join-Path $rtStage 'Data\zhCN\patch-zhCN-c.MPQ')
