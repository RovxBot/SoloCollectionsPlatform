...
class WardrobeProjectionContractTests(unittest.TestCase):
    ...
    def test_config_template_documents_sell_price_copper(self):
        ...
        self.assertIn("SoloCollections.Transmog.MixedWeapons = family", config)
        self.assertIn("Transmogrification.AllowPoor = 1", config)
        self.assertIn("Transmogrification.AllowCommon = 1", config)
        self.assertIn("Transmogrification.AllowMixedWeaponHandedness = 0", config)
    ...
    def test_collected_visual_uses_wardrobe_mixed_armor_policy(self):
        ...
        self.assertIn("IsCollectedVisualSourceClassAllowed", header)
        self.assertIn("IsCollectedVisualSourceWeaponSkillAllowed", header)
        ...
        self.assertIn('"SoloCollections.Transmog.MixedWeapons", "family"', transmog)
        self.assertIn("ITEM_SUBCLASS_WEAPON_FIST", transmog)
        self.assertIn("ITEM_SUBCLASS_WEAPON_BOW", transmog)
        self.assertIn("ITEM_SUBCLASS_WEAPON_GUN", transmog)
        self.assertIn("ITEM_SUBCLASS_WEAPON_CROSSBOW", transmog)
        self.assertIn("IsItemTransmogrifiable(source, player->GetGUID())", transmog)
        self.assertIn("IsCollectedVisualSourceClassAllowed(player, source)", transmog)
        self.assertIn("IsCollectedVisualSourceWeaponSkillAllowed(player, source)", transmog)
        ...
        self.assertIn("classEligibleSource", service)
        self.assertIn("weaponSkillEligibleSource", service)
        self.assertIn('return "CLASS_RESTRICTED";', service)
        self.assertIn('return "SKILL_REQUIRED";', service)
